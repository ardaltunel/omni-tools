    ...
    static shape(player, piece) {
        ...
        return `
            <span class="chess-symbol ${player}" aria-hidden="true">
                <img src="tools/chess-coach/img/chesspieces/wikipedia/${filename}" alt="">
            </span>
        `;
    }
...
class Game {
    ...
    availableAiActions(player = this.turn) {
        const ids = player === "WHITE" ? this.board.pieceIdsWhite : this.board.pieceIdsBlack;
        return ids.flatMap((pieceId) => {
            const position = this.board.piecePositions[pieceId];
            if (!position.active) {
                return [];
            }
            const piece = this.board.pieces[pieceId];
            const moves = this.board.piecesTilesMoves[pieceId].map((to) => ({
                pieceId,
                piece,
                from: {row: position.row, col: position.col},
                to,
                capture: false
            }));
            const captures = this.board.piecesTilesCaptures[pieceId].map((option) => {
                const targetLocation = option.capture || option;
                const capturedPiece = this.board.tileFind(targetLocation);
                return {
                    pieceId,
                    piece,
                    from: {row: position.row, col: position.col},
                    to: {row: option.row, col: option.col},
                    capture: true,
                    capturedPiece,
                    capturedPieceId: capturedPiece === null || capturedPiece === void 0 ? void 0 : capturedPiece.data.id
                };
            }).filter((action) => action.capturedPieceId);
            return moves.concat(captures);
        });
    }

    aiActionScore(action, difficulty = "normal") {
        const values = {PAWN: 100, KNIGHT: 320, BISHOP: 330, ROOK: 500, QUEEN: 900, KING: 20000};
        const file = Utils.colToInt(action.to.col);
        const rank = Utils.rowToInt(action.to.row);
        const distanceFromCenter = Math.abs(file - 3.5) + Math.abs(rank - 3.5);
        const ownValue = values[action.piece.data.type] || 0;
        let score = 0;

        if (action.capture && action.capturedPiece) {
            const capturedValue = values[action.capturedPiece.data.type] || 0;
            score += capturedValue * 1.4 - ownValue * 0.12;
        }

        score += (7 - distanceFromCenter) * 12;

        if (action.piece.data.type === "PAWN") {
            const direction = action.piece.playerWhite() ? 1 : -1;
            score += direction * (Utils.rowToInt(action.to.row) - Utils.rowToInt(action.from.row)) * 18;
        }

        if ((action.piece.data.type === "KNIGHT" || action.piece.data.type === "BISHOP") && action.piece.moves.length === 0) {
            score += 28;
        }

        const opponentCaptures = action.piece.playerWhite()
            ? this.board.tilesPiecesBlackCaptures
            : this.board.tilesPiecesWhiteCaptures;
        const defendedByOpponent = opponentCaptures[action.to.row][action.to.col].length;
        if (defendedByOpponent) {
            score -= difficulty === "hard" ? ownValue * 0.75 : ownValue * 0.35;
        }

        if (action.piece.data.type === "KING" && action.piece.moves.length === 0 && (action.to.col === "C" || action.to.col === "G")) {
            score += 70;
        }
    ...
    pickAiAction(difficulty = "normal") {
        const actions = this.availableAiActions(this.turn);
        if (!actions.length) {
            return null;
        }
        if (difficulty === "easy") {
            const captures = actions.filter((action) => action.capture);
            const pool = captures.length && Math.random() < 0.35 ? captures : actions;
            return pool[Math.floor(Math.random() * pool.length)];
        }

        const ranked = actions
            .map((action) => ({action, score: this.aiActionScore(action, difficulty)}))
...
class View {
    ...
    handleTileClick(location) {
        ...
        if ((type === "MOVE" || type === "CAPTURE") && this.onMove) {
            this.onMove({type, activePieceId, capturedPieceId});
        }
        // crazy town
    ...
    playAiAction(action) {
        ...
        this.game.activePieceOptions = [];
        this.drawResetClassNames();
        if (action.capture) {
            this.game.capture(action.pieceId, action.capturedPieceId, action.to);
            this.drawCapturedPiece(action.capturedPieceId);
        } else {
            this.game.move(action.pieceId, action.to);
        }
        this.drawPiecePositions([action.from, action.to], "");
        if (this.onMove) {
            this.onMove({type: action.capture ? "CAPTURE" : "MOVE", activePieceId: action.pieceId, capturedPieceId: action.capturedPieceId});
        }
    }
...
class ChessController {
    constructor(game, view) {
        ...
        this.level = "normal";
        this.moveCount = 0;
        this.status = document.getElementById("chess-status");
        this.turn = document.getElementById("chess-turn");
        this.moves = document.getElementById("chess-moves");
        this.levelText = document.getElementById("chess-level-text");
        this.botText = document.getElementById("chess-bot-text");
        this.levelButtons = Array.from(document.querySelectorAll(".chess-level"));
        this.newButton = document.getElementById("chess-new");

        this.view.onMove = this.afterMove.bind(this);
        this.levelButtons.forEach((button) => {
            button.addEventListener("click", () => this.setLevel(button.dataset.level || "normal"));
        });
        this.newButton === null || this.newButton === void 0 ? void 0 : this.newButton.addEventListener("click", () => window.initChessTool());
    ...
    setLevel(level) {
        ...
        this.levelButtons.forEach((button) => button.classList.toggle("active", button.dataset.level === level));
        if (this.levelText) {
            this.levelText.textContent = labels[level] || "Normal";
        }
    }
    ...
    updateStatus(text) {
        if (this.moves) {
            this.moves.textContent = Math.ceil(this.moveCount / 2).toString();
        }
        if (this.turn) {
            this.turn.textContent = this.game.turn === "WHITE" ? "Beyaz" : "Siyah";
        }
        if (this.botText) {
            this.botText.textContent = "Siyah";
        }
        if (this.status) {
            this.status.textContent = text || (this.game.turn === "WHITE" ? "Sıra sende. Beyaz taşları oynat." : "Bot hamle düşünüyor.");
        }
    }

    scheduleBotMove() {
        this.view.locked = true;
        this.updateStatus("Bot hamle düşünüyor.");
        window.setTimeout(() => {
            const action = this.game.pickAiAction(this.level);
            this.view.playAiAction(action);
            this.view.locked = false;
            this.updateStatus(action ? "Sıra sende. Beyaz taşları oynat." : "Botun hamlesi kalmadı.");
        }, this.level === "easy" ? 360 : this.level === "hard" ? 620 : 480);
    }
}

window.initChessTool = function initChessTool() {
    const boardElement = document.getElementById("chess-board");
    if (!boardElement) {
        return;
    }
    boardElement.innerHTML = "";
    const game = new Game(Utils.getInitialPieces(), Utils.getInitialPiecePositions(), "WHITE");
    const view = new View(boardElement, game, "WHITE");
    window.chessTool = new ChessController(game, view);
};

window.addEventListener("DOMContentLoaded", window.initChessTool);
